...
def mutil_stage_train():
    ...
    if len(data.train_mask.shape) > 1: # 'texas','wisconsin','cornell','washington'
        train_mask, val_mask, test_mask = data.train_mask[:,args.split_id], data.val_mask[:,args.split_id], data.test_mask[:,args.split_id]
    else:
