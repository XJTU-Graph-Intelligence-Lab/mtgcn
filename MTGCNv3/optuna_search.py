...
def mutil_stage_train(args, trial):
    ...
    for s in range(4):   
        ...
        if s == 0:    
            pseudo_mask, easy_idx, predict, max_acc = train(data, label, train_mask, val_mask, test_mask, pseudo_mask, num_k, stage=s, args=args)
        else:
            pseudo_mask, easy_idx, predict, max_acc = train(data, label, train_mask, val_mask, test_mask, pseudo_mask, num_k, s, predict, args=args)
